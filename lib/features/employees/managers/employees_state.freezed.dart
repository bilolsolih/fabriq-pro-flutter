...
mixin _$EmployeesState {
 ...
 List<Employee> get employees; EmployeesStatus get status;
...
@useResult
$Res call({
 List<Employee> employees, EmployeesStatus status
});
...
as List<Employee>,status: null == status ? _self.status : status // ignore: cast_nullable_to_non_nullable
as EmployeesStatus,
  ));
}
...
class _EmployeesState implements EmployeesState {
  const _EmployeesState({required final  List<Employee> employees, required this.status}): _employees = employees;
